...
def compara_r(lst, compare, indice):
    if len(lst) <= 1:
        return lst
    meio = len(lst) // 2
    metade_esquerda = lst[:meio]
    metade_direita = lst[meio:]
    esquerda_ordenada = compara_r(metade_esquerda, compare, indice)
    direita_ordenada = compara_r(metade_direita, compare, indice)
    return r.merge(esquerda_ordenada, direita_ordenada, compare, indice)
...
def main():
    ...
    arquivo_saida=saida(merge,"bdmergesort-nr.txt")
    #--------------------------------------------------
    merge=compara_r(lista,descrecente,5)
    arquivo_saida=saida(merge,"bdmergesort-r.txt")
    merge=compara_r(lista,crescente,4)
    arquivo_saida=saida(merge,"bdmergesort-r.txt")
    merge=compara_r(lista,descrecente,2)
    arquivo_saida=saida(merge,"bdmergesort-r.txt")
main()
